...
class PaymentController extends MY_Controller
{
    ...
    {
        $data['title'] = 'Payments'; // Capitalize the first letter

        $this->load->view('templates/header', $data);
        $this->load->view('accounting/payments', $data);
        $this->load->view('templates/footer', $data);
    }

    public function paymentForm()
    {
        $data['title'] = 'Create Payment'; // Capitalize the first letter

        $data['classData'] = $this->model_classes->fetchClassData();

        $this->load->view('templates/header', $data);
        $this->load->view('accounting/create_payment', $data);
        $this->load->view('templates/footer', $data);
    }
    ...
    */
	...
	public function fetchType($type = null)
	{
		if($type == 1) {
			...
			$div = '<form class="form-horizontal" action="createIndividual" method="post" id="createIndividualForm">	    	
                ...
                <div class="col-sm-10">
                    ...
                    <div class="kv-avatar center-block" style="width:100%">
                        <input type="file" id="attachment" name="attachment"
                             accept="image/jpeg,image/png,application/pdf,image/jpg"  class="file-loading"/>
                    </div>
                </div>
            </div>
			<div class="form-group">
			...
			$div = '<form class="form-horizontal" action="createBulk" method="post" id="createBulkForm">	    	
	...
	*/
	public function createIndividual() 
	{
		...
		if($this->form_validation->run() === true) {
            $filePath = '';
            upload($_FILES['attachment'],'assets/uploads/payments',$filePath);
			$create = $this->model_accounting->createIndividual($filePath);
			if($create === true) {
	...
	*/
	public function fetchPaymentData()
	{
		...
		foreach ($paymentData as $key => $value) {
            $attachment = '-';
            if($value['attachment'])
                $attachment = '<a href="'.base_url() . $value['attachment'].'" target="_blank">Download</a>';
	...
	*/
	public function fetchUpdatePaymentForm($type = null)
	{
		...
		if($type == 1) {
			$option = '<form class="form-horizontal" action="updatePayment" method="post" id="updatePaymentFrom">	    	
		  	<div class="form-group">
	...
	*/
	public function fetchStudentPaymentInfo($paymentId = null)
	{
		if($paymentId) {
			...
			<form class="form-horizontal" action="updateStudentPay" method="post" id="updateStudentPayForm">
      		<div class="col-md-6">
      			<div class="form-group">
