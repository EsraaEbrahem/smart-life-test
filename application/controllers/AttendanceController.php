...
class AttendanceController extends MY_Controller
{
    ...
    /**
     * get total attendances in a date range
     * @return void
     */
    public function attendancesCalc()
    {
        $attendances = array();
        $result = $this->model_attendance->attendancesCalc();
        foreach ($result as $calc){
            $attendances[]=array(
                'title' => 'Attendances: '.$calc['day_total'],
                'start' => $calc['attendance_date'],
                'end' => $calc['attendance_date'],
            );
        }
    ...
    public function takeAttendanceForm()
    {
        $data['title'] = 'Attendance';
        $this->load->view('templates/header', $data);
        $this->load->view('reports/add_attendance', $data);
        $this->load->view('templates/footer', $data);
    }
